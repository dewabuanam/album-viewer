...
interface Grid8PageProps {
  content: PageContent
}

export function Grid8Page({ content }: Grid8PageProps) {
        ...
        <motion.div variants={containerVariants} className="grid gap-2 sm:gap-3 md:gap-4 grid-cols-4 flex-1">
          {images.slice(0, 8).map((image, index) => (
            <motion.div
              ...
              className="relative h-16 sm:h-24 md:h-32 rounded-lg overflow-hidden cursor-pointer"
