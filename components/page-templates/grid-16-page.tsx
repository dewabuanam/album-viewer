...
interface Grid16PageProps {
  content: PageContent
}

export function Grid16Page({ content }: Grid16PageProps) {
  ...
  return (
    <>
      ...
      >
        <motion.div variants={containerVariants} className="grid gap-2 sm:gap-3 md:gap-4 grid-cols-4 flex-1">
          {images.slice(0, 16).map((image, index) => (
            <motion.div
              key={index}
              variants={itemVariants}
              whileHover={{ scale: 1.08 }}
              className="relative h-12 sm:h-16 md:h-24 rounded-lg overflow-hidden cursor-pointer"
